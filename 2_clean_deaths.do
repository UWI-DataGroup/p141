...
** LOAD DATASET  
***************
use "`datapath'\version04\2-working\2020_deaths_prepped_dp"
...
replace record_id=3077 if record_id==2497 & redcap_event_name=="tracking_arm_2"
...
sort regnum district
count if event==1 & dupreg>0  //30 - different DAs entered same certificates
...
drop if record_id==2241|record_id==118|record_id==2393|record_id==2255|record_id==2326 ///
		|record_id==2581|record_id==2561|record_id==2262|record_id==2372 //9 deleted
drop dupreg
...
count if event==1 & regexm(pname,"NIL")|event==1 & regexm(pname,"ND") //112 - all correct
...
sort pname
count if event==1 & dup>0  //2 - different DAs entered same certificates
sort pname record_id
//list record_id dddoa ddda odda pname regnum district nrn if event==1 & dup>0
replace namematch=1 if record_id==2733|record_id==3047 //2 changes
...
** (15) invalid 
count if event==1 & regexm(address, "[a-z]") //94
...
count if event==1 & regexm(address,"NIL")|event==1 & regexm(address,"ND") //238 - all correct
...
count if sex==1 & nrn==. //32 - check if there is a stata check for this e.g. soundex,etc - 0 changes
...
count if nrn!=. & age!=. & agetxt!=6 //2 - 1 correct
...
count if event==1 & dod>regdate //7 (check redcapdb for if any coroner cases; 2019 TF redcap report for 'true' 2019 cases)
...
replace dod=dod2 if dod2!=. //7 changes
replace regdate=regdate2 if regdate2!=. //7 changes
drop dod2 regdate2
...
preserve
clear
import excel using "`datapath'\version04\1-input\NRN_20200909.xlsx" , firstrow case(lower)
save "`datapath'\version04\2-working\2020_deaths_nrn" ,replace
restore
merge 1:1 record_id using "`datapath'\version04\2-working\2020_deaths_nrn" ,force
...
count if natregno!="" & length(natregno)!=10 //16
//list record_id ddda odda pname age nrn natregno if natregno!="" & length(natregno)!=10
replace flag18=flag18+1 if natregno!="" & length(natregno)!=10 //16 changes
...
replace corr_AH=corr_AH+1 if record_id==2815|record_id==2856 //2 changes
replace corr_KG=corr_KG+1 if record_id==2660|record_id==2713 //2 changes
...
count if regexm(natregno,"9999")|regexm(natregno,"99") //7 (checked against electoral list)
...
count if sex==2 & (regex(substr(natregno,-2,1), "[1,3,5,7,9]")) & !(strmatch(strupper(natregno), "*-9999*")) //6
...
recode sex 2=1 if record_id==2266|record_id==3071 //1 change
replace flag12=flag12+1 if record_id==2266|record_id==3071 //1 change
replace corr_intern=corr_intern+1 if record_id==2266
replace corr_KG=corr_KG+1 if record_id==3071
** (40) invalid - male with 'female' NRN
count if sex==1 & regex(substr(natregno,-2,1), "[0,2,4,6,8]") //7
//list record_id ddda odda pname natregno sex cod* if sex==1 & regex(substr(natregno,-2,1), "[0,2,4,6,8]")
recode sex 1=2 if record_id==281|record_id==339 //2 changes
...
replace corr_KG=corr_KG+1 if record_id==281 //1 change
...
count if (agetxt!=6 & natregno!="")|regex(substr(dobyr,1,1),"[0]") //5
...
replace dobyr="19"+dobyr if length(dobyr)==2 //1291 changes
...
replace flag18=flag18+1 if record_id==186|record_id==420|record_id==2399 //3 changes
...
count if age!=age2 & natregno!="" & dob!=. //46
sort record_id
...
** (44) invalid 
count if event==1 & regexm(occu, "[a-z]") //86
...
count if event==1 & regexm(occu,"NIL")|event==1 & regexm(occu,"ND") //93
//list record_id ddda odda occu if event==1 & regexm(occu,"NIL")|event==1 & regexm(occu,"ND")
count if event==1 & occu=="NIL"|event==1 & occu=="ND" //22
//list record_id ddda odda occu if event==1 & occu=="NIL"|event==1 & occu=="ND"
replace flag20=flag20+1 if event==1 & occu=="NIL"|event==1 & occu=="ND" //22 changes
replace corr_intern=corr_intern+1 if event==1 & ddda==98 & occu=="NIL"|event==1 & occu=="ND" //15 changes
replace corr_AH=corr_AH+1 if event==1 & ddda==25 & occu=="NIL"|event==1 & occu=="ND" //7 changes
replace occu="99" if event==1 & occu=="NIL"|event==1 & occu=="ND" //22 changes
...
** (46) invalid
count if durationnum==999 & durationtxt!=. & durationtxt!=99 //47
...
count if durationnum==999 & durationtxt==99 //308 - correct
replace durationtxt=. if durationnum==999 & durationtxt==99 //308 changes
...
count if event==1 & regexm(cod1a,"NIL")|event==1 & regexm(cod1a,"ND") //98 - all correct
...
** (52) possibly invalid
count if onsetnumcod1a==999 & onsettxtcod1a!=. & onsettxtcod1a!=99 //64
...
count if onsetnumcod1a==999 & onsettxtcod1a==99 //443
replace onsettxtcod1a=. if onsetnumcod1a==999 & onsettxtcod1a==99 //443 changes
...
count if event==1 & regexm(cod1b,"NIL")|event==1 & regexm(cod1b,"ND") //53 - all correct
...
** (58) possibly invalid
count if onsetnumcod1b==999 & onsettxtcod1b!=. & onsettxtcod1b!=99 //48
...
** (60) possibly invalid
count if onsetnumcod1b!=999 & onsettxtcod1b==99 //74
...
count if onsetnumcod1b==999 & onsettxtcod1b==99 //343
replace onsettxtcod1b=. if onsetnumcod1b==999 & onsettxtcod1b==99 //343 changes
...
count if event==1 & regexm(cod1c,"NIL")|event==1 & regexm(cod1c,"ND") //19 - all correct
...
** (66) possibly invalid
count if onsetnumcod1c!=999 & onsettxtcod1c==99 //131
list record_id ddda duration* onsetnumcod1c onsettxtcod1c if onsetnumcod1c!=999 & onsettxtcod1c==99
replace flag28=flag28+1 if onsetnumcod1c!=999 & onsettxtcod1c==99 //131 changes
replace onsetnumcod1c=999 if onsetnumcod1c!=999 & onsettxtcod1c==99 //131 changes
count if onsetnumcod1c==999 & onsettxtcod1c==99 //224
replace onsettxtcod1c=. if onsetnumcod1c==999 & onsettxtcod1c==99 //224 changes
...
count if event==1 & regexm(cod1d,"NIL")|event==1 & regexm(cod1d,"ND") //5 - all correct
...
** (70) possibly invalid
count if onsetnumcod1d==999 & onsettxtcod1d!=. & onsettxtcod1d!=99 //11
...
** (72) possibly invalid
count if onsetnumcod1d!=999 & onsettxtcod1d==99 //152
//list record_id ddda duration* onsetnumcod1d onsettxtcod1d if onsetnumcod1d!=999 & onsettxtcod1d==99
replace flag34=flag34+1 if onsetnumcod1d!=999 & onsettxtcod1d==99 //152 changes
replace onsetnumcod1d=999 if onsetnumcod1d!=999 & onsettxtcod1d==99 //152 changes
count if onsetnumcod1d==999 & onsettxtcod1d==99 //181
replace onsettxtcod1d=. if onsetnumcod1d==999 & onsettxtcod1d==99 //181 changes
...
count if event==1 & regexm(cod2a,"NIL")|event==1 & regexm(cod2a,"ND") //28 - all correct
...
** (76) possibly invalid
count if onsetnumcod2a==999 & onsettxtcod2a!=. & onsettxtcod2a!=99 //55
...
count if onsetnumcod2a==999 & onsettxtcod2a==99 //301
replace onsettxtcod2a=. if onsetnumcod2a==999 & onsettxtcod2a==99 //301 changes
...
count if event==1 & regexm(cod2b,"NIL")|event==1 & regexm(cod2b,"ND") //8 - all correct
...
** (82) possibly invalid
count if onsetnumcod2b==999 & onsettxtcod2b!=. & onsettxtcod2b!=99 //20
...
** (84) possibly invalid
count if onsetnumcod2b!=999 & onsettxtcod2b==99 //125
//list record_id ddda duration* onsetnumcod2b onsettxtcod2b if onsetnumcod2b!=999 & onsettxtcod2b==99
replace flag40=flag40+1 if onsetnumcod2b!=999 & onsettxtcod2b==99 //125 changes
...
count if onsetnumcod2b==999 & onsettxtcod2b==99 //226
replace onsettxtcod2b=. if onsetnumcod2b==999 & onsettxtcod2b==99 //226 changes
...
count if event==1 & regexm(pod,"NIL")|event==1 & regexm(pod,"ND") //88 - all correct
...
*/
** (88) Christ Church
count if deathparish==1 & district!=1 //67 - all correct
...
replace flag42=flag42+1 if record_id==406
replace district=2 if record_id==406 //1 change
** (91) St James
...
replace flag42=flag42+1 if record_id==185|record_id==2568 //2 changes
...
** (97) St Philip
count if deathparish==10 & district!=3 //2
//list record_id ddda deathparish district pod if deathparish==10 & district!=3
replace flag42=flag42+1 if record_id==2420|record_id==2517 //2 changes
replace corr_KG=corr_KG+1 if record_id==2420 //1 change
replace corr_intern=corr_intern+1 if record_id==2517 //1 change
replace district=3 if record_id==2420|record_id==2517 //2 changes
** (98) St Thomas
...
count if event==1 & regexm(certifier,"NIL")|event==1 & regexm(certifier,"ND") //123 - all correct
...
count if event==1 & regexm(certifieraddr,"NIL")|event==1 & regexm(certifieraddr,"ND") //41 - all correct
...
** (105) missing
count if event==1 & namematch==. //1339 - already checked for duplicates above
replace namematch=2 if event==1 & namematch==. //1339 changes
//list record_id ddda namematch if event==1 & namematch==.


** cleaned: 1=Yes; 2=No: FLAG 47
** (105) missing
count if event==1 & cleaned==. //1329
count if event==1 & cleaned==2 //12
//tab cleaned if event==1 ,m
replace cleaned=1 if event==1 & (cleaned==.|cleaned==2) //1341 changes
...
replace corr_intern=corr_intern+1 if event==1 & recstatdc==0 //1 change
...
** (109) missing
count if event==2 & tfdddoa==. //1
//list record_id tfdddoaend if event==2 & tfdddoa==.
replace tfdddoa=tfdddoaend if event==2 & tfdddoa==. //1 change
...
** tfddtxt: FLAG 60
replace tfddtxt = rtrim(ltrim(itrim(tfddtxt))) //35 changes
...
save "`datapath'\version04\3-output\2020_deaths_cleaned_dqi_dc" ,replace
...
label data "BNR MORTALITY data 2020"
notes _dta :These data prepared from BB national death register & BNR (Redcap) deathdata database
save "`datapath'\version04\3-output\2020_deaths_cleaned_export_dc" ,replace
